...
function normalizeFeedback(rawMessage, source) {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return {
      ...
      status: 'new',
      source: source || 'lily-chat',
    };
  }

  return {
    title: typeof parsed.title === 'string' && parsed.title.trim() ? parsed.title.trim() : 'Mr P feedback',
    category: typeof parsed.category === 'string' && parsed.category.trim() ? parsed.category.trim() : 'general',
    priority: typeof parsed.priority === 'string' && parsed.priority.trim() ? parsed.priority.trim() : 'normal',
    summary: typeof parsed.summary === 'string' && parsed.summary.trim() ? parsed.summary.trim() : 'General feedback',
    details: typeof parsed.details === 'string' && parsed.details.trim() ? parsed.details.trim() : 'General feedback',
    user_message: typeof parsed.user_message === 'string' ? parsed.user_message.trim() : '',
    status: typeof parsed.status === 'string' && parsed.status.trim() ? parsed.status.trim() : 'new',
    source: source || 'lily-chat',
  };
}
...
async function writeToGoogleDrive(record) {
  ...
  const filenameSafeTitle = record.title.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '-').slice(0, 60) || 'feedback';
